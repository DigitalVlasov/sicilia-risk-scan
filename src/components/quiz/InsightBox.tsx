...
export const InsightBox: React.FC<InsightBoxProps> = ({ 
  ...
  const applyKeyTermBolding = (text: string): string => {
    ...
    processedText = processedText.replace(/\b(analisi gratuita|consulenza gratuita|valutazione gratuita|scopri come|vedi come|analisi completa)\b/gi, '<a href="#contact-cta" class="inline-flex items-center gap-1 bg-red-600 hover:bg-red-700 text-white font-bold px-3 py-1 rounded-md transition-all duration-200 shadow-sm hover:shadow-md hover:scale-105 no-underline">$1 →</a>');
    
    // Convert action phrases to more prominent CTAs
    processedText = processedText.replace(/\b(prenota ora|richiedi ora|contattaci|parlane con noi|fissa un appuntamento)\b/gi, '<a href="#contact-cta" class="inline-flex items-center gap-1 bg-green-600 hover:bg-green-700 text-white font-bold px-4 py-2 rounded-lg transition-all duration-200 shadow-md hover:shadow-lg hover:scale-105 no-underline">$1 →</a>');
